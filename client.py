...
function send() {
    ...
    if (this.readyState == 4 && this.status == 200) {
        ...
        for (i = 0; i < data.length; i++) {
            ...
            try {
                from_lat = Number(data[i]["start"].split(",")[0]);
                from_lng = Number(data[i]["start"].split(",")[1]);
                dest_lat = Number(data[i]["dest"].split(",")[0]);
                dest_lng = Number(data[i]["dest"].split(",")[1]);
    ...
    xhttp.send(JSON.stringify({"content": [document.getElementById("name").value, document.getElementById("origin").value, document.getElementById("dest").value, document.getElementById("radius").value]}));
}
</script>
...
function initialize() {
    var myOptions = {
        center: {lat: 43.463409, lng: -80.603024},
        zoom: 8
    };
...
<input type="text" id="origin" name="origin" placeholder="Your starting point...">
<label for="dest">Destination</label>
<input type="text" id="dest" name="dest" placeholder="Your destination...">
<label for="radius">Radius (km)</label><br>
<input type="number" id="radius" name="radius"><br>
<button id="button" onclick="send()">Go!</button>
